...
const state = { page: 1, pageSize: 10, search: "", kategori: "", jenisKelamin: "", totalPages: 1 };
...
async function loadTable() {
  ...
  try {
    const data = await SIPIL.call("getData", {
      ...
      search: state.search, kategori: state.kategori, jenisKelamin: state.jenisKelamin,
    });
...
function openEdit(r) {
  ...
  document.getElementById("editJenisKelamin").value = r.jenisKelamin;
...
document.getElementById("filterJenisKelamin").addEventListener("change", (e) => {
  state.jenisKelamin = e.target.value; state.page = 1; loadTable();
});
...
async function exportToExcel() {
  ...
  try {
    ...
    ws["!cols"] = [
      { wch: 26 }, { wch: 12 }, { wch: 22 }, { wch: 12 }, { wch: 10 },
      { wch: 14 }, { wch: 18 }, { wch: 22 }, { wch: 28 }, { wch: 20 },
    ];
